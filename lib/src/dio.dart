...
abstract class DioMixin implements Dio {
  ...
  Future<Response<T>>? reject<T>(err) {
    ...
    } as FutureOr<Response<T>> Function(dynamic), onError: (e) {
  ...
  Future<Response<T>> _request<T>(
    ...
    interceptors.forEach((Interceptor interceptor) {
      future = future.then(_interceptorWrapper(interceptor.onRequest, true) as FutureOr<dynamic> Function(dynamic));
    });

    // Add dispatching callback to request flow
    future = future.then(_interceptorWrapper(_dispatchRequest, true) as FutureOr<dynamic> Function(dynamic));

    // Add response interceptors to request flow
    interceptors.forEach((Interceptor interceptor) {
      future = future.then(_interceptorWrapper(interceptor.onResponse, false) as FutureOr<dynamic> Function(dynamic));
    });
  ...
  Future<Response<T>?> _dispatchRequest<T>(RequestOptions options) async {
    ...
    try {
      ...
      if (statusOk) {
        return checkIfNeedEnqueue(interceptors.responseLock, () => ret) as FutureOr<Response<T>?>;
      } else {
        throw DioError(
          response: ret,
  ...
  Future<T> listenCancelForAsyncTask<T>(
    ...
    return Future.any([
      if (cancelToken != null)
        cancelToken.whenCancel.then(((e) => throw cancelToken.cancelError!) as FutureOr<T> Function(void)),
      future,
    ]);
  }
